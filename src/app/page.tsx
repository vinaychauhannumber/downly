// Server Component — no 'use client'
// All static SEO content (H1, FAQs, tools grid, guides grid) is pre-rendered
// in HTML so Googlebot can read it without executing JavaScript.

import React from 'react';
import type { Metadata } from 'next';
import Link from 'next/link';
import { Header } from '@/components/Header';
import { Hero } from '@/components/Hero';
import { HomeDownloader } from '@/components/HomeDownloader';
import { PlatformCards } from '@/components/PlatformCards';
import { HowItWorks } from '@/components/HowItWorks';
import { Features } from '@/components/Features';
import { FAQ } from '@/components/FAQ';
import { Footer } from '@/components/Footer';
import { StructuredData } from '@/components/StructuredData';
import { AdBanner } from '@/components/AdBanner';
import { SEO_TOOLS } from '@/lib/seo-data';
import { GUIDES_DATA } from '@/lib/guides-data';
import { ArrowRight, Sparkles, BookOpen } from 'lucide-react';

// Page-level metadata (overrides layout defaults for the homepage).
// Tuned for primary search intents: "youtube video downloader",
// "instagram reels downloader", "youtube mp3 downloader".
export const metadata: Metadata = {
  title: 'Free YouTube Video & Instagram Reels Downloader – Downly',
  description:
    'Download YouTube videos in 1080p Full HD MP4 or convert to 320kbps MP3 audio. Save Instagram Reels and videos in HD. 100% free, no watermark, no login.',
  alternates: {
    canonical: 'https://downlyfree.onrender.com',
  },
  openGraph: {
    title: 'Free YouTube Video & Instagram Reels Downloader – Downly',
    description:
      'Download YouTube videos in 1080p MP4 or 320kbps MP3. Save Instagram Reels in HD. Free, fast, no watermark.',
    url: 'https://downlyfree.onrender.com',
    siteName: 'Downly',
    type: 'website',
    images: [
      {
        url: '/og-image.png',
        width: 1200,
        height: 630,
        alt: 'Downly – Free YouTube Video & Instagram Reels Downloader',
      },
    ],
  },
  twitter: {
    card: 'summary_large_image',
    title: 'Free YouTube Video & Instagram Reels Downloader – Downly',
    description:
      'Download YouTube videos in 1080p MP4 or 320kbps MP3. Save Instagram Reels in HD. Free, fast, no watermark.',
    creator: '@downly',
  },
};

// Static FAQ data — defined here so it's embedded in server-rendered HTML
// AND fed into the FAQPage JSON-LD schema.
const homepageFaqs = [
  {
    question: 'What platforms are supported by Downly?',
    answer:
      'Downly currently supports publicly accessible YouTube videos, YouTube Shorts, Instagram Reels, and Instagram video posts in full compliance with public access policies.',
  },
  {
    question: 'Can I download 1080p Full HD YouTube videos with sound?',
    answer:
      'Yes! Downly merges separate 1080p Full HD video streams and high-fidelity AAC stereo audio into a single, perfectly synchronized MP4 file with zero audio lag.',
  },
  {
    question: 'Can I convert YouTube videos to 320kbps MP3 audio?',
    answer:
      'Yes. Switch the format to MP3 Audio and choose up to 320 kbps studio quality bitrate with embedded ID3 metadata tags.',
  },
  {
    question: 'Is Downly free and safe to use?',
    answer:
      'Downly is 100% free with no registration, no adware, and no desktop executable software required. All processing is securely executed in the cloud.',
  },
];

export default function HomePage() {
  const toolsList = Object.values(SEO_TOOLS);
  const guidesList = Object.values(GUIDES_DATA).slice(0, 4);

  return (
    <div className="min-h-screen bg-[#08090d] text-slate-100 flex flex-col bg-grid-pattern selection:bg-cyan-500 selection:text-black">
      {/* Schema.org Structured Data — rendered in server HTML */}
      <StructuredData
        appName="Downly – Free YouTube Video & Instagram Reels Downloader"
        appDescription="Download public Instagram Reels, feed videos, YouTube videos, and Shorts in 1080p MP4 or 320kbps MP3 audio."
        appUrl="https://downlyfree.onrender.com"
        faqItems={homepageFaqs}
      />

      {/* Top Header */}
      <Header />

      <main className="flex-1">
        {/* Hero Section with primary H1 — server-rendered */}
        <Hero />

        {/* Top Ad Unit */}
        <AdBanner slot="1000000001" label="Sponsored" />

        {/* Interactive Downloader Widget (Client Component) */}
        <HomeDownloader />

        {/* Supported Platforms Section — server-rendered */}
        <PlatformCards />

        {/* Mid-Page Ad Unit */}
        <AdBanner slot="1000000002" label="Advertisement" />

        {/* Dedicated High-Intent Downloader Tools Grid — server-rendered */}
        <section
          className="py-16 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 border-t border-white/5"
          aria-label="Specialized Downloader Tools"
        >
          <div className="text-center max-w-3xl mx-auto mb-12 space-y-3">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 text-xs font-semibold uppercase tracking-wider">
              <Sparkles className="w-3.5 h-3.5" aria-hidden="true" />
              <span>Dedicated Downloader Tools</span>
            </div>
            <h2 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
              Fast, Specialized Tools for Every Media Type
            </h2>
            <p className="text-sm sm:text-base text-slate-400">
              Access optimized download pipelines tailored specifically for Instagram Reels,
              YouTube videos, vertical Shorts, and MP3 audio conversions.
            </p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {toolsList.map((tool) => (
              <Link
                key={tool.slug}
                href={`/${tool.slug}`}
                className="group p-6 rounded-3xl bg-slate-950/60 border border-white/10 hover:border-cyan-500/40 hover:bg-white/[0.02] transition-all flex flex-col justify-between"
              >
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="px-2.5 py-1 rounded-md bg-cyan-500/10 text-cyan-400 text-[11px] font-semibold uppercase">
                      {tool.badge}
                    </span>
                    <ArrowRight className="w-4 h-4 text-slate-500 group-hover:text-cyan-400 group-hover:translate-x-1 transition-all" aria-hidden="true" />
                  </div>

                  <h3 className="text-base font-bold text-white group-hover:text-cyan-300 transition-colors">
                    {tool.h1}
                  </h3>

                  <p className="text-xs text-slate-400 leading-relaxed line-clamp-2">
                    {tool.subtitle}
                  </p>
                </div>

                <div className="pt-4 border-t border-white/5 mt-4 text-[11px] font-semibold text-cyan-400 flex items-center gap-1">
                  <span>Open Tool</span>
                  <ArrowRight className="w-3 h-3" aria-hidden="true" />
                </div>
              </Link>
            ))}
          </div>
        </section>

        {/* How It Works Section — server-rendered */}
        <HowItWorks />

        {/* Core Features — server-rendered */}
        <Features />

        {/* SEO Guides Section — server-rendered */}
        <section
          className="py-16 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 border-t border-white/5"
          aria-label="Download Guides and Tutorials"
        >
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4">
            <div>
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 text-xs font-semibold uppercase tracking-wider mb-2">
                <BookOpen className="w-3.5 h-3.5" aria-hidden="true" />
                <span>Knowledge &amp; Step-by-Step Guides</span>
              </div>
              <h2 className="text-2xl sm:text-3xl font-extrabold text-white tracking-tight">
                Latest Download Tutorials
              </h2>
            </div>

            <Link
              href="/guides"
              className="text-xs font-bold text-cyan-400 hover:text-cyan-300 flex items-center gap-1.5 shrink-0"
            >
              <span>View All Guides ({Object.keys(GUIDES_DATA).length})</span>
              <ArrowRight className="w-3.5 h-3.5" aria-hidden="true" />
            </Link>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {guidesList.map((guide) => (
              <Link
                key={guide.slug}
                href={`/guides/${guide.slug}`}
                className="group p-5 rounded-2xl bg-slate-950/50 border border-white/5 hover:border-cyan-500/30 hover:bg-white/[0.02] transition-all flex flex-col justify-between"
              >
                <div className="space-y-2.5">
                  <span className="px-2 py-0.5 rounded bg-cyan-500/10 text-cyan-400 text-[10px] font-semibold uppercase">
                    {guide.category}
                  </span>
                  <h3 className="text-xs font-bold text-white group-hover:text-cyan-300 transition-colors line-clamp-2">
                    {guide.h1}
                  </h3>
                  <p className="text-[11px] text-slate-400 line-clamp-2 leading-relaxed">
                    {guide.summary}
                  </p>
                </div>

                <div className="pt-3 border-t border-white/5 mt-4 text-[11px] text-slate-500 font-mono">
                  {guide.readTime}
                </div>
              </Link>
            ))}
          </div>
        </section>

        {/* FAQ Accordion — server-rendered */}
        <FAQ />
      </main>

      {/* Footer */}
      <Footer />
    </div>
  );
}
